...
class TwoColumn(TestCaseBase):
    ...
    def generate_gcode(self):
        ...
        retraction_prime = self.params["retraction_prime"]["value"]
        retraction_jump = self.params["retraction_jump"]["value"]
        travel_speed = self.params["travel_speed"]["value"]
        ...
        while gcode_generator.total_z < height:
            ...
            # Define points for first column
            p1 = (center1_x + (tmp_size / 2), center1_y + (tmp_size / 2))
            p2 = (center1_x + (tmp_size / 2), center1_y - (tmp_size / 2))
            p3 = (center1_x - (tmp_size / 2), center1_y - (tmp_size / 2))
            p4 = (center1_x - (tmp_size / 2), center1_y + (tmp_size / 2))

            # Define points for second column
            p5 = (center2_x + (tmp_size / 2), center2_y + (tmp_size / 2))
            p6 = (center2_x + (tmp_size / 2), center2_y - (tmp_size / 2))
            p7 = (center2_x - (tmp_size / 2), center2_y - (tmp_size / 2))
            p8 = (center2_x - (tmp_size / 2), center2_y + (tmp_size / 2))
            ...
            # Retraction with Z-hop and travel to the second column
            gcode_generator.retract(retraction, retraction_jump)
            gcode_generator.move(p5[0], p5[1],  travel_speed)
            gcode_generator.retract(-retraction_prime, -retraction_jump)
            ...
            # Retraction with Z-hop and travel back to the first column
            gcode_generator.retract(retraction, retraction_jump)
            gcode_generator.move(p1[0], p1[1], travel_speed)
            gcode_generator.retract(-retraction_prime, -retraction_jump)
    ...
    def estimate_print_time(self):
        # Simple estimation logic based on distance and height
        length = self.params["distance"]["value"]
        height = self.params["height"]["value"]
